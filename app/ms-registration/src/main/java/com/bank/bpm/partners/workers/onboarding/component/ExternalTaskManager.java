...
public class ExternalTaskManager {
	...
	public static class Builder {
		...
		public ExternalTaskManager build() {
			try {
				...
				this.listeners.forEach( handler -> {
					ExternalTaskComponent annotation = handler.getClass().getAnnotation(ExternalTaskComponent.class);

					final TopicSubscription subscription = client.subscribe(annotation.topic())
							.handler(ExternalTaskHandlerDecorator.from(handler, settings)).open();

					this.subscriptions.add(subscription);
				});

				return new ExternalTaskManager(client, settings, subscriptions);
